...
logisticRegressionClinicalOdds<-function(pheno=NULL,full.dn4=NULL,phenotypeColumn=NULL,mycFeature=NULL,groupA=1){
  ...
  d<-coef(summary(fit))
 d<-cbind(d,confint(fit))
 colnames(d)<-c("Estimate","Std.Error","z.value","p.value","lower.2.5","upper.97.5")
	return(d)
  } 
...
color_clusters <- c("#DC050C", "#FB8072", "#1965B0", "#7BAFDE", "#882E72", 
 ...
 colorCodeUMAP.SpatialPhenoColors<-function(full.dn4=NULL){
  ...
  umapColors<-umapColors[!grepl("Tumor_11",rownames(umapColors)),]
...
### Spatial analysis using the sub-clusters
 This analyzes the sub-cluster spatial.
...
########################################
 ...
 inth.tumor<-Heatmap(interact[,grepl("Tumor",rownames(interact))],
  ...
  input<-zz%>%group_by(tumor.5nn.class.unsup)%>%summarise

allAssembleNeighborhoodExpression(zz=zz,jointDesign,
	phenotypeCommunity="tumor.5nn.class.unsup",groupBy="case")
...
### ANOVA model over the labeled TME and tumor topoology clusters
```{r, eval=FALSE}
 ...
 ### calculate any expression differences across sub-clusters and treatment response.
 ###   
 subExpr<-zz%>%group_by(ROIID,tumor.5nn.umap.simple.label)%>%summarise(CXCR3=mean(Cell_CXCR3),
	CCR4=mean(Cell_CCR4),
	BCL2=mean(Cell_BCL2),
	BCL6=mean(Cell_BCL6),
	CD30=mean(Cell_CD30),
	...
	## remove none tumor cluster.
	subExpr[,2]<-factor(subExpr[,2])
   	subExpr<-subExpr[which(subExpr[,2]!='none'),]
  	subExpr[,2]<-droplevels(subExpr[,2])
	
	alpha<-data.frame(feat=levels(subExpr$tumor.5nn.umap.simple.label),alpha=0)
 	alpha<-data.frame(model.matrix(~0+feat,alpha))
	colnames(alpha)<-gsub("feat","",colnames(alpha))

	####	total levels is 26
	alpha_noint <- rbind(
	grandMean=rep(1,26)/26,
	CD45RA_CD4.vs.mean=ifelse(subExpr[,2]%>%levels=="CD45RA_CD4",25,-1)/26,
	CD45RA_exhaustedCD4.vs.mean=ifelse(subExpr[,2]%>%levels=="CD45RA_exhaustedCD4",25,-1)/26,
	CD45RANeg_exhaustedCD4.vs.mean=ifelse(subExpr[,2]%>%levels=="CD45RANeg_exhaustedCD4",25,-1)/26,
	CD45RA_TIM3_CD4.vs.mean=ifelse(subExpr[,2]%>%levels=="CD45RA_TIM3_CD4",25,-1)/26,
	Ki67_memoryCD4.vs.mean=ifelse(subExpr[,2]%>%levels=="Ki67_memoryCD4",25,-1)/26,
	MemoryCD4.vs.mean=ifelse(subExpr[,2]%>%levels=="MemoryCD4",25,-1)/26,
	#CD8
	CD8.vs.mean=ifelse(subExpr[,2]%>%levels=="CD8",25,-1)/26,
	ExhaustedCD8.vs.mean=ifelse(subExpr[,2]%>%levels=="ExhaustedCD8",25,-1)/26,
	Ki67_CD8.vs.mean=ifelse(subExpr[,2]%>%levels=="Ki67_CD8",25,-1)/26,	
	## topology
	ta.vs.mean=ifelse(subExpr[,2]%>%levels=="Tumor_a",25,-1)/26,
	tb.vs.mean=ifelse(subExpr[,2]%>%levels=="Tumor_b",25,-1)/26,
	tc.vs.mean=ifelse(subExpr[,2]%>%levels=="Tumor_c",25,-1)/26,
	td.vs.mean=ifelse(subExpr[,2]%>%levels=="Tumor_d",25,-1)/26,
	te.vs.mean=ifelse(subExpr[,2]%>%levels=="Tumor_e",25,-1)/26,
	tf.vs.mean=ifelse(subExpr[,2]%>%levels=="Tumor_f",25,-1)/26,
	tg.vs.mean=ifelse(subExpr[,2]%>%levels=="Tumor_g",25,-1)/26,
	th.vs.mean=ifelse(subExpr[,2]%>%levels=="Tumor_h",25,-1)/26,
	ti.vs.mean=ifelse(subExpr[,2]%>%levels=="Tumor_i",25,-1)/26,

	##TREG
	suptreg.vs.mean=ifelse(subExpr[,2]%>%levels=="HighlysuppressiveTreg",25,-1)/26,
	Ki67_Treg.vs.mean=ifelse(subExpr[,2]%>%levels=="Ki67_Treg",25,-1)/26,
	Treg.vs.mean=ifelse(subExpr[,2]%>%levels=="Treg",25,-1)/26,
	#MAC
	Ki67_M1Macrophage.vs.mean=ifelse(subExpr[,2]%>%levels=="Ki67_M1Macrophage",25,-1)/26,
	M1Macrophage.vs.mean=ifelse(subExpr[,2]%>%levels=="M1Macrophage",25,-1)/26,
	M2Macrophage.vs.mean=ifelse(subExpr[,2]%>%levels=="M2Macrophage",25,-1)/26,
	PDL1_M2Macrophage.vs.mean=ifelse(subExpr[,2]%>%levels=="PDL1_M2Macrophage",25,-1)/26,
	##compare M1.vs M2
	M2.vs.M1=c(rowSums(alpha[,c("PDL1_M2Macrophage","M2Macrophage")])/2-rowSums(alpha[,c("Ki67_M1Macrophage","M1Macrophage")])/2),	
	Endothelial.vs.mean=ifelse(subExpr[,2]%>%levels=="Endothelial",25,-1)/26)
 ...
 cmyc.global <- lmer(cMYC ~ 0+unsup.subcommunity +(1|case), data = subExpr)
  ...
  tim3.global.ci$pvalues<-xx$test$pvalues
	...
	##write to FIGURE2
	library(openxlsx)
 	  fig6<-loadWorkbook("~/Documents/imageAnalysis/DLBCL/simpleModel-1-14/Figure6-CD8-5NN-spatialModel-RICO/tumor.cd8.dlbcl/Figure6-final/Figure6Table.xlsx")
	openxlsx::addWorksheet(fig6,"labeledTopol.estimates")
	openxlsx::writeData(fig6,'labeledTopol.estimates',
	globalExpr,
	rowNames=TRUE,withFilter=FALSE,colNames=TRUE,startCol=1,startRow=1)
	openxlsx::saveWorkbook(fig6,file="~/Documents/imageAnalysis/DLBCL/simpleModel-1-14/Figure6-CD8-5NN-spatialModel-RICO/tumor.cd8.dlbcl/Figure6-final/Figure6Table2.xlsx",overwrite=TRUE)
...
library(RColorBrewer)
 ...
 ## perform the spatial interactions 
 full.dn4$tumor.5nn.umap.simple.label<-as.character(full.dn4$annotated.umap.simple)
 ...
 ### CD4
  full.dn4$tumor.5nn.umap.relabel<-as.character(full.dn4$tumor.5nn.umap.simple.label)
  table(full.dn4$tumor.5nn.umap.relabel)
 full.dn4$tumor.5nn.umap.relabel[which(full.dn4$annotated.umap.simple=='CD45RA_CD4')]<-as.character(full.dn4$annotated.umap[which(full.dn4$annotated.umap.simple=='CD45RA_CD4')])
  full.dn4$tumor.5nn.umap.relabel[which(full.dn4$annotated.umap.simple=='CD45RA_exhaustedCD4')]<-as.character(full.dn4$annotated.umap[which(full.dn4$annotated.umap.simple=='CD45RA_exhaustedCD4')])

  full.dn4$tumor.5nn.umap.relabel[which(full.dn4$annotated.umap.simple=='CD45RANeg_exhaustedCD4')]<-as.character(full.dn4$annotated.umap[which(full.dn4$annotated.umap.simple=='CD45RANeg_exhaustedCD4')])

 full.dn4$tumor.5nn.umap.relabel[which(full.dn4$annotated.umap.simple=='CD8')]<-as.character(full.dn4$annotated.umap[which(full.dn4$annotated.umap.simple=='CD8')])

full.dn4$tumor.5nn.umap.relabel[which(full.dn4$annotated.umap.simple=='CD45RA_TIM3_CD4')]<-as.character(full.dn4$annotated.umap[which(full.dn4$annotated.umap.simple=='CD45RA_TIM3_CD4')])

full.dn4$tumor.5nn.umap.relabel[which(full.dn4$annotated.umap.simple=='ExhaustedCD8')]<-as.character(full.dn4$annotated.umap[which(full.dn4$annotated.umap.simple=='ExhaustedCD8')])

full.dn4$tumor.5nn.umap.relabel[which(full.dn4$annotated.umap.simple=='HighlysuppressiveTreg')]<-as.character(full.dn4$annotated.umap[which(full.dn4$annotated.umap.simple=='HighlysuppressiveTreg')])

full.dn4$tumor.5nn.umap.relabel[which(full.dn4$annotated.umap.simple=='Ki67_CD8')]<-as.character(full.dn4$annotated.umap[which(full.dn4$annotated.umap.simple=='Ki67_CD8')])

full.dn4$tumor.5nn.umap.relabel[which(full.dn4$annotated.umap.simple=='Ki67_M1Macrophage')]<-as.character(full.dn4$annotated.umap[which(full.dn4$annotated.umap.simple=='Ki67_M1Macrophage')])

full.dn4$tumor.5nn.umap.relabel[which(full.dn4$annotated.umap.simple=='Ki67_memoryCD4')]<-as.character(full.dn4$annotated.umap[which(full.dn4$annotated.umap.simple=='Ki67_memoryCD4')])

full.dn4$tumor.5nn.umap.relabel[which(full.dn4$annotated.umap.simple=='Ki67_Treg')]<-as.character(full.dn4$annotated.umap[which(full.dn4$annotated.umap.simple=='Ki67_Treg')])

full.dn4$tumor.5nn.umap.relabel[which(full.
dn4$annotated.umap.simple=='M1Macrophage')]<-as.character(full.dn4$annotated.umap[which(full.dn4$annotated.umap.simple=='M1Macrophage')])

full.dn4$tumor.5nn.umap.relabel[which(full.dn4$annotated.umap.simple=='M2Macrophage')]<-as.character(full.dn4$annotated.umap[which(full.dn4$annotated.umap.simple=='M2Macrophage')])

full.dn4$tumor.5nn.umap.relabel[which(full.dn4$annotated.umap.simple=='MemoryCD4')]<-as.character(full.dn4$annotated.umap[which(full.dn4$annotated.umap.simple=='MemoryCD4')])

full.dn4$tumor.5nn.umap.relabel[which(full.dn4$annotated.umap.simple=='PDL1_M2Macrophage')]<-as.character(full.dn4$annotated.umap[which(full.dn4$annotated.umap.simple=='PDL1_M2Macrophage')])

full.dn4$tumor.5nn.umap.relabel[which(full.dn4$annotated.umap.simple=='Treg')]<-as.character(full.dn4$annotated.umap[which(full.dn4$annotated.umap.simple=='Treg')])
 ...
 # can be slow.
  ...
  dat_relation_15<-createRelationTable(dn5=full.dn4,
	...
	minDistance=15)
  save(dat_relation_15,file="dat_relation_15_tumor.5nn.umap.simple.label.15distance.RData")
 ...
 ## using the p=0.01 threshold.
  ...
  save(p1,file="p1_15_annotatedLabel.tumor.5nn.umap.simple.label.15distance.025.RData")
 ...
 ## load the interactions
 load("~/Documents/imageAnalysis/DLBCL/simpleModel-1-14/final-submission-folder/figure6-final/p1_15_annotatedLabel.tumor.5nn.umap.simple.label.15distance.025.RData")
 ## heatmap 
  interact<-p1$pmat
 # use the labels.
 rownames(interact)<-full.dn4$tumor.5nn.umap.relabel[match(rownames(interact),full.dn4$tumor.5nn.umap.simple.label)]
 colnames(interact)<-full.dn4$tumor.5nn.umap.relabel[match(colnames(interact),full.dn4$tumor.5nn.umap.simple.label)]
 ...
 jointDesign<-jointDesign[,which(colnames(jointDesign)!="BCL6")]
  ...
  input<-  allAssembleNeighborhoodExpression(zz=zz,jointDesign,
	phenotypeCommunity="tumor.5nn.umap.simple.label",groupBy="case")

 input<-input[which(input$Var1!='none'),]
 ...
 ## perform the multivariate logistic regression on the labeled data sets.
 
  tum<-as.data.frame.matrix(table(full.dn4$case,full.dn4$tumor.5nn.umap.simple.label))
  tum<-tum/rowSums(tum)
 # tum<-as.data.frame(tum[,grepl("Tumor_",colnames(tum))])
  imm<-tum
  imm$case<-rownames(tum)
  imm$IPI<-full.dn4$IPI[match(imm$case,full.dn4$case)]
  imm$NGCB<-full.dn4$COO[match(imm$case,full.dn4$case)]
  imm$NGCB<-ifelse(imm$NGCB=="NGCB",1,0)
   ##flip the model estimate to match figure 2
 imm$GCB<-ifelse(imm$NGCB==1,0,1)


 ## NGCB OR!
 ## TUmor
  hans.mlr<- glm(NGCB~Tumor_a+Tumor_b+Tumor_c+Tumor_d+Tumor_e+Tumor_f+Tumor_g+Tumor_h+Tumor_i+IPI,
	family='binomial',imm)%>%summary

 ### perform HANS logistic regression for each primary phenotype in multivariate setting.
 ### only
   cd4.mlr<- glm(NGCB~CD45RA_CD4+CD45RA_exhaustedCD4+CD45RANeg_exhaustedCD4+CD45RA_TIM3_CD4+Ki67_memoryCD4+MemoryCD4+IPI,family='binomial',imm)%>%summary
 
 
 ## CD8
   cd8.mlr<- glm(NGCB~CD8+ExhaustedCD8+Ki67_CD8+IPI,family='binomial',imm)%>%summary
 
  treg.mlr<- glm(NGCB~HighlysuppressiveTreg+Ki67_Treg+Treg+IPI,family='binomial',imm)%>%summary
  

  mac.mlr<- glm(NGCB~Ki67_M1Macrophage+M1Macrophage+M2Macrophage+PDL1_M2Macrophage+IPI,family='binomial',imm)%>%summary
...
#### final MLR after selecting best from each TME block.
 ...
 cvfit = cv.glmnet(x, y, family = "binomial", type.measure = "class")
  ...
  selected.mlr<- glm(GCB~Tumor_e+Tumor_g+M2Macrophage+Ki67_CD8++CD45RANeg_exhaustedCD4+Endothelial,family='binomial',imm)%>%summary


###mutational fitted model using labeled phenotypes.
  mutFit<-empiricalFitMutationCaseLevel(full.dn4=full.dn4,
	design=jointDesign,
	phenotypeCommunity="tumor.5nn.umap.simple.label",
	toDrop=c("HighlysuppressiveTreg",
	"Ki67_Treg",
	'Treg',
	"Endothelial",'none'))
 ...
 ##TP53
  library(circlize)
  tp53Data<-data.frame(phenotype=unique(full.dn4$tumor.5nn.umap.simple.label))
  pvalue_col_fun = colorRamp2(c(0, 2, 3), c("grey95", "grey95", "grey95")) 
   tp53Data$tp53.symbol=NA
  tp53Data$logFC<-0
    tp53Data$direction<-0
 tp53Data$adj.P.Val<-1
  rownames(tp53Data)<-tp53Data[,1]
  tp53Data<-tp53Data[,-1]
 ...
 tp53Data<-appendDE(coef=2,mutation="TP53",tp53Data=tp53Data)
  ...
  ### simplify the Tumor_c to only CD79B and TP53/CD79B
  tp53Data['Tumor_c',c("BCL6.CD79B.symbol")]<-NA
  tp53Data['Tumor_c',c("BCL6.CD79B.logFC")]<-0
  tp53Data['Tumor_c',c("BCL6.CD79B.direction")]<-0
  tp53Data['Tumor_c',c("BCL6.CD79B.adj.P.Val")]<-1
  tp53Data<-tp53Data[which(rownames(tp53Data)!='none'),]

 
 rownames(tp53Data)<-full.dn4$tumor.5nn.umap.relabel[match(rownames(tp53Data),full.dn4$tumor.5nn.umap.simple.label)]

 ###append the empirical bayesian model logFc and adjPVAL
  tp53Data[rownames(interact),]
...
###
  ...
  tp53HA<-HeatmapAnnotation("CD79b+"=anno_empty(border=TRUE,
	...
	"BCL6+CD79b+"=anno_simple(tp531$BCL6.CD79B.direction,
	pch=tp531$BCL6.CD79B.symbol,
	border=TRUE,
	col=pvalue_col_fun,
	pt_gp=gpar(col=c("white"))),
	show_annotation_name=FALSE,
	...
	col=pvalue_col_fun,
	pt_gp=gpar(col=c("white"))),
	"BCL6+CD79B+"=anno_empty(border=TRUE,
	height= unit(5, "mm"),
	),	
	show_annotation_name=TRUE,
	which="column")
 ...
 ## include the double expressor MYC+BCL2/BCL6 information.
...
### the double expressors information IHC.
 ## double expressors annotation.
  ...
  full.dn4$MYC.Rearrange=0
   full.dn4$MYC.Rearrange<-myc$MYC.Rearrange[match(full.dn4$ROIID,myc$ROIID)]
  full.dn4$MYC.Gain=0
  full.dn4$MYC.Gain<-myc$MYC.Gain[match(full.dn4$ROIID,myc$ROIID)]
 ## add in 9p24 status
 ...
 ### glmOdds
  dEx.Odds<-lapply(as.character(unique(full.dn4$tumor.5nn.umap.simple.label)),function(x) logisticRegressionClinicalOdds(pheno=x,full.dn4,
	phenotypeColumn="tumor.5nn.umap.simple.label",mycFeature="double.expressor",groupA=1))
  
   names(dEx.Odds)<-as.character(unique(full.dn4$tumor.5nn.umap.simple.label))
    
  # mycData<-data.frame(glm.OR=sapply(dEx.Odds,function(x) x[2,]),row.names=names(dEx.Odds))
  mycData<-sapply(dEx.Odds,function(x) x[2,c("Estimate","p.value")])%>%data.frame
  ...
  mycData[which(rownames(mycData)=="Endothelial"),]<-1
  mycData$p<-1
  mycData<-mycData[which(rownames(mycData)!='none'),]
 rownames(mycData)<-full.dn4$tumor.5nn.umap.relabel[match(rownames(mycData),full.dn4$tumor.5nn.umap.simple.label)]

  require(circlize)
 pvalue_col_fun = colorRamp2(c(0, 2, 3), c("grey95", "grey95", "grey95")) 
 mycData$symbol=NA
  mycData$symbol[which(mycData$glm.OR>2)]<-8
  mycData<-mycData[rownames(immune.interact),]
...
#################
 ## multivariate HANS association.
 ## GLM
  glmOdds<-lapply(as.character(unique(full.dn4$tumor.5nn.umap.simple.label)),function(x) logisticRegressionClinicalOdds(pheno=x,full.dn4,phenotypeColumn="tumor.5nn.umap.simple.label",mycFeature="COO",groupA="GCB"))
  names(glmOdds)<-as.character(unique(full.dn4$tumor.5nn.umap.simple.label))

   ## the logistic regression odds matches the Chi-square directionality.
   #ors<-data.frame(OR=sapply(glmOdds,function(x) x[2]),row.names=names(glmOdds))
    ors<-data.frame(OR=sapply(glmOdds,function(x) x[2,c("Estimate","p.value")]))
    ors<-as.data.frame(t(ors))
  rownames(ors)<-names(glmOdds)

  ORS<-data.frame(OR=rep(0,nrow(interact)),row.names=rownames(interact))
  ORS$p<-1
   final.drop<-selected.mlr
  final.drop<- final.drop%>%coef%>%data.frame

  final.drop<-final.drop[!rownames(final.drop)%in%c("(Intercept)","IPI"),]
 rownames(final.drop)<-full.dn4$tumor.5nn.umap.relabel[match(rownames(final.drop),full.dn4$tumor.5nn.umap.simple.label)]


   ORS[match(rownames(final.drop),rownames(ORS)),'OR']<-final.drop[,"Estimate"]
   ORS[match(rownames(final.drop),rownames(ORS)),'p']<-final.drop[,"Pr...z.."]
   ORS<-ORS[match(rownames(immune.interact),rownames(ORS)),]
...
#### row annotation color phenotype column.
 ...
 ipi.perc<- table(full.dn4$tumor.5nn.umap.relabel,full.dn4$IPI.category)%>%as.data.frame.matrix()
  ipi.perc<-100*ipi.perc/(1+rowSums(ipi.perc))
   require(circlize)
    col_fun<-colorRamp2(c(5,40,75),c("blue","white","red"))
 ...
 ht<-draw(reArrange+hans+ipis+native+inth+inth.tumor,annotation_legend_list=list(lgd.hans,lgd.myc))

   
  decorate_annotation("HANS",{
	grid.lines(0, c(0.5,26.5), gp = gpar(lty = 2),
        default.units = "native")
	popViewport()
	}) 
   
  savePlot(file="~/Documents/imageAnalysis/DLBCL/simpleModel-1-14/Figure6-CD8-5NN-spatialModel-RICO/tumor.cd8.dlbcl/Figure6-final/Figure6.spatial.analysis.mutation.15micron.MLR.labeled.025.png")

  ## DE in LR were not significant and the cmyc protein was not expressed in tumors well.
 ## dropping DE

  ht<-draw(hans+ipis+native+inth+inth.tumor,
annotation_legend_list=list(lgd.hans))
...
#	})
  ...
  savePlot(file="~/Documents/imageAnalysis/DLBCL/simpleModel-1-14/Figure6-CD8-5NN-spatialModel-RICO/tumor.cd8.dlbcl/Figure6-final/Figure6.spatial.analysis.mutation.15micron.MLR.glmNET.revised.png")
 ...
 ## write all the table data to excel.
   library(openxlsx)
   wb<-openxlsx::loadWorkbook("~/Documents/imageAnalysis/DLBCL/simpleModel-1-14/Figure6-CD8-5NN-spatialModel-RICO/tumor.cd8.dlbcl/Figure6-final/Figure6Table.xlsx")
  openxlsx::addWorksheet(wb,"mutation.labeled")
  openxlsx::writeData(wb,"mutation.labeled",
	tp53Data,rowNames=TRUE,withFilter=FALSE,colNames=TRUE,startCol=1,startRow=1)
  
  openxlsx::addWorksheet(wb,"GCB.HANS.MLR.labeled")
  openxlsx::writeData(wb,"GCB.HANS.MLR.labeled",
	ORS,rowNames=TRUE,withFilter=FALSE,colNames=TRUE,startCol=1,startRow=1)
  
  openxlsx::addWorksheet(wb,"ClusterExpression.labeled")
  openxlsx::writeData(wb,"ClusterExpression.labeled",
	input,
	rowNames=TRUE,withFilter=FALSE,colNames=TRUE,startCol=1,startRow=1)
  openxlsx::addWorksheet(wb,"InteractionScores.labeled")
  openxlsx::writeData(wb,"InteractionScores.labeled",
	interact,
	rowNames=TRUE,withFilter=FALSE,colNames=TRUE,startCol=1,startRow=1)

openxlsx::addWorksheet(wb,"IPI.labeled")
  openxlsx::writeData(wb,"IPI.labeled",
	ipi.perc,rowNames=TRUE,withFilter=FALSE,colNames=TRUE,startCol=1,startRow=1)

 openxlsx::saveWorkbook(wb,file="~/Documents/imageAnalysis/DLBCL/simpleModel-1-14/Figure6-CD8-5NN-spatialModel-RICO/tumor.cd8.dlbcl/Figure6-final/Figure6Table3.xlsx",overwrite=TRUE)
 
#####################

 # construct and re-label the bar graphs on the labeled phenotypes.
...
### PD-1 on tumor topology and labeled clusters
 PD-1 expression global anova on the labeled clusters.

```{r,eval=FALSE}
    full.dn4$geography.labels<-as.character(full.dn4$tumor.5nn.umap.simple.label)
 	full.dn4$geography.labels[which(full.dn4$geography.labels=='Tumor_d')]<-'Tumor core'
	 full.dn4$geography.labels[which(full.dn4$geography.labels=='Tumor_f')]<-'Tumor mantle'
 	full.dn4$geography.labels[which(full.dn4$geography.labels=='Tumor_i')]<-'Tumor transition zone'
 	full.dn4$geography.labels[which(full.dn4$geography.labels=='Tumor_c')]<-'Tumor crust'
 
	full.dn4$geography.labels[which(full.dn4$geography.labels=='Tumor_a' |full.dn4$geography.labels=='Tumor_b' | full.dn4$geography.labels=='Tumor_e' | full.dn4$geography.labels=='Tumor_g'| full.dn4$geography.labels=='Tumor_h' )]<-'Tumor dispersed'
...
ggplot(communityExp4,aes(x=tumor.5nn.class.unsup,y=PD1,fill=tumor.5nn.class.unsup))+geom_boxplot()+scale_fill_manual(values=spatCol[match(levels(communityExp4[,2]),spatCol[,1]),2])+theme_ipsum(base_family='Arial')
 ...
 ### calculate any expression differences across sub-clusters and treatment response.
 ###   
  subExpr<-zz%>%group_by(ROIID,geography.labels)%>%summarise(CXCR3=mean(Cell_CXCR3),
	CCR4=mean(Cell_CCR4),
	BCL2=mean(Cell_BCL2),
	BCL6=mean(Cell_BCL6),
	CD30=mean(Cell_CD30),
	...
	####	total levels is 37
	alpha_noint <- rbind(
	grandMean=rep(1,22)/22,
	CD45RA_CD4.vs.mean=ifelse(subExpr[,2]%>%levels=="CD45RA_CD4",21,-1)/22,
	CD45RA_exhaustedCD4.vs.mean=ifelse(subExpr[,2]%>%levels=="CD45RA_exhaustedCD4",21,-1)/22,
	CD45RANeg_exhaustedCD4.vs.mean=ifelse(subExpr[,2]%>%levels=="CD45RANeg_exhaustedCD4",21,-1)/22,
	CD45RA_TIM3_CD4.vs.mean=ifelse(subExpr[,2]%>%levels=="CD45RA_TIM3_CD4",21,-1)/22,
	Ki67_memoryCD4.vs.mean=ifelse(subExpr[,2]%>%levels=="Ki67_memoryCD4",21,-1)/22,
	MemoryCD4.vs.mean=ifelse(subExpr[,2]%>%levels=="MemoryCD4",21,-1)/22,
	CD8.vs.mean=ifelse(subExpr[,2]%>%levels=="CD8",21,-1)/22,
	ExhaustedCD8.vs.mean=ifelse(subExpr[,2]%>%levels=="ExhaustedCD8",21,-1)/22,
	Ki67_CD8.vs.mean=ifelse(subExpr[,2]%>%levels=="Ki67_CD8",21,-1)/22,	
	## 
	core.vs.mean=ifelse(subExpr[,2]%>%levels=="Tumor core",21,-1)/22,
	crust.vs.mean=ifelse(subExpr[,2]%>%levels=="Tumor crust",21,-1)/22,
	dispersed.vs.mean=ifelse(subExpr[,2]%>%levels=="Tumor dispersed",21,-1)/22,
	mantle.vs.mean=ifelse(subExpr[,2]%>%levels=="Tumor mantle",21,-1)/22,
	tz.vs.mean=ifelse(subExpr[,2]%>%levels=="Tumor transition zone",21,-1)/22,
	
	suptreg.vs.mean=ifelse(subExpr[,2]%>%levels=="HighlysuppressiveTreg",21,-1)/22,
	Ki67_Treg.vs.mean=ifelse(subExpr[,2]%>%levels=="Ki67_Treg",21,-1)/22,
	Treg.vs.mean=ifelse(subExpr[,2]%>%levels=="Treg",21,-1)/22,
	
	Ki67_M1Macrophage.vs.mean=ifelse(subExpr[,2]%>%levels=="Ki67_M1Macrophage",21,-1)/22,
	M1Macrophage.vs.mean=ifelse(subExpr[,2]%>%levels=="M1Macrophage",21,-1)/22,
	M2Macrophage.vs.mean=ifelse(subExpr[,2]%>%levels=="M2Macrophage",21,-1)/22,
	PDL1_M2Macrophage.vs.mean=ifelse(subExpr[,2]%>%levels=="PDL1_M2Macrophage",21,-1)/22,
	
	Endothelial.vs.mean=ifelse(subExpr[,2]%>%levels=="Endothelial",21,-1)/22)


 	library(multcomp);library(lme4)
	pd1.global <- lmer(PD1 ~ 0+geography.labels +(1|case), data = subExpr)
 	pdl1.global <- lmer(PDL1 ~ 0+geography.labels +(1|case), data = subExpr)
 	ki67.global <- lmer(Ki67 ~ 0+geography.labels +(1|case), data = subExpr)
  	cd30.global <- lmer(CD30~ 0+geography.labels +(1|case), data = subExpr)
	...
	pdl1.global.ci$marker<-'PDL1'
	##Ki67
	 ki67.global <- glht(ki67.global, linfct=alpha_noint)
 	 ki67.global.ci<-confint(ki67.global, calpha=univariate_calpha())
  	ki67.global.ci<- ki67.global.ci$confint%>%data.frame
  	xx<-ki67.global%>%summary
  	ki67.global.ci$pvalues<-xx$test$pvalues
	ki67.global.ci$marker<-'Ki67'
	##
	##CD30

  cd30.global <- glht(cd30.global, linfct=alpha_noint)
	...
	cd30.global.ci)
 	...
 	library(openxlsx)
 	  fig6<-loadWorkbook("~/Documents/imageAnalysis/DLBCL/simpleModel-1-14/Figure6-CD8-5NN-spatialModel-RICO/tumor.cd8.dlbcl/Figure6-final/Figure6Table.xlsx")
	addWorksheet(fig6,"Topol.Expression.estimates")
	writeData(fig6,'Topol.Expression.estimates',
	topoExpr,
	rowNames=TRUE,withFilter=FALSE,colNames=TRUE,startCol=1,startRow=1)

   	addWorksheet(fig6,"Topol.MLR.GCB")
	writeData(fig6,'Topol.MLR.GCB',
	 selected.mlr%>%coef,
	rowNames=TRUE,withFilter=FALSE,colNames=TRUE,startCol=1,startRow=1)

	addWorksheet(fig6,"Topol.Dbl.Expr")
	writeData(fig6,'Topol.Dbl.Expr',
	 data.frame(p.value=sapply(dEx.Odds,function(x) x[2,'p.value'])),
	rowNames=TRUE,withFilter=FALSE,colNames=TRUE,startCol=1,startRow=1)
 ...
 ## Tumor PD-1
  tumor.pd1<-pd1.global.ci[c("core.vs.mean",
	"crust.vs.mean",
	"dispersed.vs.mean",
	"mantle.vs.mean",
	"tz.vs.mean"),]
  tumor.pd1$label<-c("Core","Crust","Dispersed","Mantle","Transition zone")
  
 
 tumor.pd1$label<-factor(tumor.pd1$label,levels=c("Core","Mantle","Transition zone","Crust","Dispersed"))

  tumor.pd1%>%ggplot(aes(x=label,y=Estimate))+geom_point(aes(size=6),show.legend=FALSE)+geom_errorbar(aes(ymin=lwr,ymax=upr),width=0.2,colour='black')+geom_hline(yintercept=0,colour='red',linetype='dashed')+theme_ipsum(base_family='Arial')+theme(axis.text.x=element_text(angle=45,size=18,color='black',hjust=1),axis.title.x=element_text(size=15),axis.text.y=element_text(colour='black',size=15),axis.title.y=element_text(size=15))+xlab("Tumor topology")+ylab("Tumor PD-1 mean intensity (Z-score)")

  ggsave("~/Documents/imageAnalysis/DLBCL/simpleModel-1-14/final-submission-folder/figure6-final/PD1.tumor.summary.topology.png")


 # Tumor PD-1
  tumor.pdl1<-pdl1.global.ci[c("core.vs.mean",
	"crust.vs.mean",
	"dispersed.vs.mean",
	"mantle.vs.mean",
	"tz.vs.mean"),]
  tumor.pdl1$label<-c("Core","Crust","Dispersed","Mantle","Transition zone")
  
 
 tumor.pdl1$label<-factor(tumor.pdl1$label,levels=c("Core","Mantle","Transition zone","Crust","Dispersed"))

  tumor.pdl1%>%ggplot(aes(x=label,y=Estimate))+geom_point(aes(size=6),show.legend=FALSE)+geom_errorbar(aes(ymin=lwr,ymax=upr),width=0.2,colour='black')+geom_hline(yintercept=0,colour='red',linetype='dashed')+theme_ipsum(base_family='Arial')+theme(axis.text.x=element_text(angle=45,size=18,color='black',hjust=1),axis.title.x=element_text(size=15),axis.text.y=element_text(colour='black',size=15),axis.title.y=element_text(size=15))+xlab("Tumor topology")+ylab("Tumor PD-L1 mean intensity (Z-score)")


 ## ki
 # Tumor 
  tumor.ki<-ki67.global.ci[c("Ki67_memoryCD4.vs.mean",
	"Ki67_CD8.vs.mean",
	"Ki67_Treg.vs.mean",
	...
	"mantle.vs.mean",
	"tz.vs.mean",
	"CD45RA_exhaustedCD4.vs.mean",
	"ExhaustedCD8.vs.mean"),]
  tumor.ki$label<-c("Ki67+Memory CD4",
	"Ki67+CD8","Ki67+TREG","Ki67+M1 MAC",
	"Core","Crust","Dispersed","Mantle","Transition zone",
	"CD45RA+ exhausted CD4",
	"Exhausted CD8")
  
 
 tumor.ki$label<-factor(tumor.ki$label,levels=c("Ki67+Memory CD4",
	"Ki67+CD8","Ki67+TREG","Ki67+M1 MAC",
	"Core","Mantle","Transition zone","Crust","Dispersed",
	"CD45RA+ exhausted CD4",
	"Exhausted CD8"))



  tumor.ki%>%ggplot(aes(x=label,y=Estimate))+geom_point(aes(size=6),show.legend=FALSE)+geom_errorbar(aes(ymin=lwr,ymax=upr),width=0.2,colour='black')+geom_hline(yintercept=0,colour='red',linetype='dashed')+theme_ipsum(base_family='Arial')+theme(axis.text.x=element_text(angle=45,size=18,color='black',hjust=1),axis.title.x=element_text(size=15),axis.text.y=element_text(colour='black',size=15),axis.title.y=element_text(size=15))+xlab("TME phenotype, Tumor topology")+ylab("Tumor Ki67 mean intensity (Z-score)")

 ggsave("~/Documents/imageAnalysis/DLBCL/simpleModel-1-14/final-submission-folder/figure6-final/Ki67.tumor.summary.topology.png")




	
   tumor.cd30<-cd30.global.ci[c("core.vs.mean",
	"crust.vs.mean",
	"dispersed.vs.mean",
	"mantle.vs.mean",
	"tz.vs.mean"),]
  tumor.cd30$label<-c("Core","Crust","Dispersed","Mantle","Transition zone")
 tumor.cd30$label<-factor(tumor.cd30$label,levels=c("Core","Mantle","Transition zone","Crust","Dispersed"))

 

  tumor.cd30%>%ggplot(aes(x=label,y=Estimate))+geom_point(aes(size=6),show.legend=FALSE)+geom_errorbar(aes(ymin=lwr,ymax=upr),width=0.2,colour='black')+geom_hline(yintercept=0,colour='red',linetype='dashed')+theme_ipsum(base_family='Arial')+theme(axis.text.x=element_text(angle=45,size=18,color='black',hjust=1),axis.title.x=element_text(size=15),axis.text.y=element_text(colour='black',size=15),axis.title.y=element_text(size=15))+xlab("Tumor topology")+ylab("Tumor CD30 mean intensity (Z-score)")

 ggsave("~/Documents/imageAnalysis/DLBCL/simpleModel-1-14/final-submission-folder/figure6-final/CD30.tumor.summary.topology.png")




   cxcr3.global <- lmer(CXCR3 ~ 0+geography.labels +(1|case), data = subExpr)
	# CXCR3
	 cxcr3.global <- glht(cxcr3.global, linfct=alpha_noint)
 	 cxcr3.global.ci<-confint(cxcr3.global, calpha=univariate_calpha())
  	cxcr3.global.ci<- cxcr3.global.ci$confint%>%data.frame
...
### CXCR3 model on the label clusters
 We examine the CXCR3 expression and the correlation with Tumor_d (core) interactions using the labeled phenotypes.
 This is Figure 6C revision.
```{r, eval=FALSE}
 #load("~/Documents/imageAnalysis/DLBCL/simpleModel-1-14/Figure6-CD8-5NN-spatialModel-RICO/tumor.cd8.dlbcl/tumor-spatial-class5NN-unsupCommunity/p1_15_Tumor.5nn.unsup.RData")
 ...
 spatCols<-getSpatialPhenoColors(full.dn4)
  ...
  patZ<-zz%>%group_by(ROIID,tumor.5nn.umap.simple.label)%>%summarise(TIM3=mean(Cell_Tim3),
	CXCR3=mean(Cell_CXCR3),
	PD1=mean(Cell_PD1),
	CCR4=mean(Cell_CCR4),
	LAG3=mean(Cell_Lag3))%>%data.frame
 cxcr3<-patZ%>%group_by(tumor.5nn.umap.simple.label)%>%summarise(CXCR3=mean(CXCR3))%>%data.frame
  ## only the CD4 CXCR3 expression.
 ...
 f.cd4<-ints['Tumor_f',grepl("CD4",colnames(ints))]
 cd4.f<-ints[match(names(f.cd4),rownames(ints)),'Tumor_f']
  avg.ints.f<- rbind(f.cd4,cd4.f)%>%colMeans



 #cd4ints<-t(totalPrimary2['core',grepl("CD4_",colnames(totalPrimary2))])%>%data.frame(tumor.5nn.class.unsup=rownames(t(totalPrimary2['core',grepl("CD4_",colnames(totalPrimary2))])))%>%data.frame
 cd4ints<-data.frame(avg.ints)
 cd4ints$tumor.5nn.umap.simple.label<-rownames(cd4ints)

  cxcr3<-left_join(cxcr3,cd4ints,by='tumor.5nn.umap.simple.label')

 



 cxcr3$CXCR3<-cxcr3.global.ci[c("CD45RA_CD4.vs.mean",
             "CD45RA_exhaustedCD4.vs.mean",
	 "CD45RANeg_exhaustedCD4.vs.mean",
	 "CD45RA_TIM3_CD4.vs.mean",
	 "Ki67_memoryCD4.vs.mean",
	"MemoryCD4.vs.mean")  ,1]

 
  cor.test( cxcr3$CXCR3,cxcr3$avg.ints)
  cxcr3$relabel<-full.dn4$tumor.5nn.umap.relabel[match(cxcr3$tumor.5nn.umap.simple.label,full.dn4$tumor.5nn.umap.simple.label)]

 phenoColors<-colorCodeUMAP.SpatialPhenoColors(full.dn4=full.dn4)

  cxc<-ggplot(cxcr3,aes(x=CXCR3,y=avg.ints,label=relabel))+geom_point(size=6,aes(fill=relabel),pch=21,colour='black')+geom_smooth(method='lm',se=FALSE,col='red')+scale_fill_manual(values=as.character(phenoColors[match(cxcr3$relabel,phenoColors$phenotype),'colors']))+xlab("CD4 CXCR3 expression (Z-score)")+ylab("Signif. tumor core interactions (%)")+theme_ipsum(base_family='Arial')+theme(axis.text.x=element_text(size=19,colour='black'),
	axis.title.x=element_text(size=17), 
	axis.text.y = element_text(size=17,colour='black'),
	axis.title.y=element_text(size=15))+ggtitle("CD4 CXCR3 expression assocation with tumor core interactions")+ggplot2::annotate('text',x=-0.51,y=4.2,label="p-value=0.12, R^2=0.69",size=5)+geom_text_repel(label=cxcr3$relabel,point.padding=0.5,size=6)
 cxc
 ggsave("~/Documents/imageAnalysis/DLBCL/simpleModel-1-14/Figure6-CD8-5NN-spatialModel-RICO/tumor.cd8.dlbcl/Figure6-final/CXCR3.core.interactions.correlation.CD4.labeled.png")

 #library(openxlsx)
 #fig6<-loadWorkbook("~/Documents/imageAnalysis/DLBCL/simpleModel-1-14/Figure6-CD8-5NN-spatialModel-RICO/tumor.cd8.dlbcl/Figure6-final/Figure6Table.xlsx")
  # addWorksheet(fig6,"CD4_5.CXCR3.LMER.labeled")
   # writeData(fig6,"CD4_5.CXCR3.LMER.labeled",
	#data.frame(estimate=  cor.test( cxcr3$CXCR3,cxcr3$avg.ints)$estimate,p= cor.test( cxcr3$CXCR3,cxcr3$avg.ints)$p.value,row.names="correlation.CD45.Tumor_d"),
	#rowNames=TRUE,withFilter=FALSE,colNames=TRUE,startCol=1,startRow=1)
...
### TME sub-cluster association with tumor neighborhood zone labeled phenotypes
This performs the neighborhood summary of the labeled phenotypes.
 We used p-value of 0.025 threshold.
```{r,eval=FALSE}
 load("~/Documents/imageAnalysis/DLBCL/simpleModel-1-14/final-submission-folder/figure6-final/p1_15_annotatedLabel.tumor.5nn.umap.simple.label.15distance.025.RData")
...
## testing out.
 ...
 all(tp$variable==lower$variable)
  ...
  tp$primary<-factor(tp$primary,levels=c("dispersed","Tumor_c","Tumor_i","Tumor_f","Tumor_d"))
  tp$penetrant<-'Dispersed'
  tp$penetrant[which(tp$primary=='Tumor_c' | tp$primary=='Tumor_i'|tp$primary=='Tumor_f')]<-'Peripheral'
  tp$penetrant[which(tp$primary=='Tumor_d')]<-'Penetrating'
  tp$penetrant<-factor(tp$penetrant,levels=c('Dispersed','Peripheral','Penetrating'))

   tp$annotate<-full.dn4$tumor.5nn.umap.relabel[match(tp$variable,full.dn4$tumor.5nn.umap.simple.label)]
...
### reduced simplified summary of boundary interactions.
 ...
 tp4$annotate<-factor(tp4$annotate,levels=c("CD45RA+ CD4",
	"Memory CD4",
	"Ki67+ memory CD4",
	"CD45RA+ exhausted CD4",
	"CD45RA- exhausted CD4",
 ...
 ggplot(tp4,aes(x=penetrant,y=value.x,fill=topography))+geom_bar(stat='identity')+geom_errorbar(aes(ymin=lower.y,ymax=upper.y),width=0.5)+facet_wrap(~annotate)+theme_ipsum(base_family="Arial")+theme(axis.text.x = element_text(angle = 90, hjust = 1,colour='black',size=14),
	axis.text.y=element_text(size=14,colour='black'),
	axis.title.y=element_text(size=14))+ylab("Case interactions (%)")+xlab("Tumor topography")+ggtitle("TME penetrating heterogeneity")+scale_fill_manual(values=c('gray49','tan3','black','khaki1','#B17BA6'))+geom_hline(yintercept=0,color='red',linetype='dashed')
 ...
 ggsave("~/Documents/imageAnalysis/DLBCL/simpleModel-1-14/Figure6-CD8-5NN-spatialModel-RICO/tumor.cd8.dlbcl/Figure6-final/labeled-revision-p025/boundary.labeled.interactions.tumor.neighborhood.rowSummary.png")
  ...
  levels(tp4$annotate)<-c("CD45RA+ CD4",
	              "Memory CD4",
	 "Ki67+ memory CD4" ,
	        "CD45RA+ exhausted CD4" ,
	 "CD45RA- exhausted CD4",
	...
	"Ki67+ CD8",
	 "CD8",
      "Exhausted CD8","Ki67+ M1-Macrophage",
	"M1-Macrophage" ,
	"M2-Macrophage",
	 "PD-L1+ M2-Macrophage", "Ki67+ T-reg",
	    "T-reg" ,
	 "Highly suppressive T-reg",
	 "Endothelial" )            
...
,],aes(x=penetrant,y=value.x,fill=topography))+geom_bar(stat='identity')+geom_errorbar(aes(ymin=lower.y,ymax=upper.y),width=0.5)+facet_wrap(~annotate)+theme_ipsum(base_family="Arial")+theme(axis.text.x = element_text(angle = 90, hjust = 1,colour='black',size=14),
	axis.text.y=element_text(size=14,colour='black'),
	axis.title.y=element_text(size=14))+ylab("Case interactions (%)")+xlab("Tumor topography")+ggtitle("TME penetrating heterogeneity")+scale_fill_manual(values=c('gray49','tan3','black','khaki1','#B17BA6'))+geom_hline(yintercept=0,color='red',linetype='dashed')
