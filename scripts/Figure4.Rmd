...
```{r, eval=FALSE}
 ...
 ## Cluster 7 was an ambiguous cluster. 
  ...
  hd.omit.id<-hd.z$uniqueLabel[which(hd.z[which(hd.z$phenograph_cluster_meta==7),"CD4"]<=(-0.7) |hd.z[which(hd.z$phenograph_cluster_meta==7),"CD3"]<=(-0.7))]
   ...
   plot_clustering_heatmap_wrapper(expr=integ2[,c("CD20","CD30","CD206","CD3","CD45RA","CD45RO","CD4","CD68","CD8a","EphrinB2","FOXP3")], 
  cell_clustering=integ2[,"phenograph_cluster_meta"],
  useMedians=FALSE,
  useQuantiles=TRUE,
  color_clusters=NULL)

 
  integ2$MC<-paste0("C",integ2$phenograph_cluster_meta)
   integ3<-metaClusterPhenotype(dn4=integ2,
	phenotype="C4",
	myPheno="MC",
	markers=colnames(integ2)[1:22],plotPCA=FALSE,k2=25)

 save(integ2,file="~/Documents/imageAnalysis/DLBCL/simpleModel-1-14/final-submission-folder/figure4-final/integ2.clustered.reproduced.RData")

 ## filter low CD4
  omitCD4.dim<-which(integ3$C4_subphenograph_cluster_meta=="1" | integ3$C4_subphenograph_cluster_meta=="4")
 integ2<-integ2[-omitCD4.dim,]
   ...
   anno$cluster.id<-seq(1,16)
  integ2$annotation<-anno[match(integ2$phenograph_cluster_meta,anno$cluster.id),"anno"]
 ...
 ### ANOVA comparing cluster differences.
  ...
  # immune<-cleandat[which(cleandat$annotation!="BCell" & cleandat$annotation!="HRS"),]
   #immune$Area<-0
   #immune$Area<-cleandat$Area[match(immune$uniqueLabel,cleandat$uniqueLabel)]
 ...
 ## selected demonstration. constant expression across ROI and experiment.
  immune%>%group_by(ROIID,experiment)%>%summarise(PDL1=mean(PDL1),CD4=mean(CD4),CD68=mean(CD68),Area=mean(Area),DNA=mean(DNA1))%>%data.frame
  
  ### filtering out extreme phenotypes.
   mac.id<-which(immune$CD68  > quantile(immune$CD68,0.965) & immune$annotation=="Macrophage")
    dna.id<-which(immune$DNA1  > quantile(immune$DNA1,0.975) & immune$annotation=="TREG")
      area.id<-which(immune$Area  > quantile(immune$Area,0.975) & immune$annotation=="TREG")

  immune[c(-mac.id,-dna.id,-area.id),]%>%group_by(ROIID,experiment,annotation)%>%summarise(CD4=median(CD4),CD68=median(CD68),FOXP3=median(FOXP3),CD8=median(CD8a),Area=median(Area),DNA=median(DNA1))%>%reshape2::melt()%>%ggplot(aes(x=annotation,y=value,fill=experiment))+geom_boxplot()+facet_wrap(~variable)+scale_fill_manual(values=getExperimentColors())+theme_bw(base_size=14)+theme(axis.text.x = element_text(angle = 45, hjust = 1))
 

 # ggsave("Lineage.marker.intensity.TME.normalized.png")
...
```{r,eval=FALSE}
   require(limma)
   immune<-cleandat[which(cleandat$annotation!="BCell" & cleandat$annotation!="HRS"),]
   immune<-immune[!immune$ROIID%in%c("Placenta_1_3_a0","Placenta_2_4_a0","Tonsil_1_1_a0","Tonsil_2_2_a0"),]
...
```{r,eval=FALSE}
 ...
 ## barplot of primary phenotypes comparing HD vs DLBCL.
  ...
  ErikMeta=immune[,
   ...
   plot_clustering_heatmap_wrapper(expr=ErikMeta[,c("CD206","CD3","CD45RA","CD45RO","CD4","CD68","CD8a","FOXP3","CXCR3","CCR4","HLADR","Ki67","PD1","PDL1","TIM3")], 
  cell_clustering=ErikMeta[,"subannotation2"],
  useMedians=FALSE,
  useQuantiles=TRUE,
  color_clusters=integratedColors)
    savePlot(file="TME.immune.HD.DLBCL.subannotation.png")
